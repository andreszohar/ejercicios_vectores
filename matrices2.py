# 2. Realice un algoritmo que permita hacer un ahorcadito, la matriz permitirá almacenar las palabras, se recomienda que las palabras tengan el mismo tamaño para evitar reasignar el valor o tamaño de la matriz.
...
def jugar_otra_vez():
    return input('Deseas jugar otra vez (introduce si para sí o cualquier otra cosa para no): ')
...
if __name__ == '__main__':
    ...
    diccionario = ['casa', 'pescado', 'calamar', 'monigote', 'codigopiton','enematosperatomorpho']

    bienvenida()
    while True:
        jugar_al_ahorcado(diccionario)
        if jugar_otra_vez() != 'si': break
    despedida()
